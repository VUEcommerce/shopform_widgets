...
typedef SFDropdownButtonBuilder = List<Widget> Function(BuildContext context);

class _SFDropdownMenuPainter extends CustomPainter {
  _SFDropdownMenuPainter({
    this.color,
    this.elevation,
    this.selectedIndex,
    this.resize,
  ...
  @override
  bool shouldRepaint(_SFDropdownMenuPainter oldPainter) {
...
class _DropdownMenuItemButton<T> extends StatefulWidget {
  ...
  final _SFDropdownRoute<T> route;
  ...
  @override
  _SFDropdownMenuItemButtonState<T> createState() =>
      _SFDropdownMenuItemButtonState<T>();
}

class _SFDropdownMenuItemButtonState<T>
  ...
  void _handleOnTap() {
    ...
    Navigator.pop(
      context,
      _SFDropdownRouteResult<T>(dropdownMenuItem.value),
    );
  }
...
class _SFDropdownMenu<T> extends StatefulWidget {
  const _SFDropdownMenu({
    Key key,
    this.padding,
    this.route,
    this.buttonRect,
  ...
  @override
  _SFDropdownMenuState<T> createState() => _SFDropdownMenuState<T>();
}

class _SFDropdownMenuState<T> extends State<_SFDropdownMenu<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    return FadeTransition(
      ...
      child: CustomPaint(
        painter: _SFDropdownMenuPainter(
...
class _SFDropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  _SFDropdownMenuRouteLayout({
    @required this.buttonRect,
    @required this.route,
  ...
  @override
  bool shouldRelayout(_SFDropdownMenuRouteLayout<T> oldDelegate) {
...
@immutable
class _SFDropdownRouteResult<T> {
  const _SFDropdownRouteResult(this.result);

  final T result;

  @override
  bool operator ==(Object other) {
    return other is _SFDropdownRouteResult<T> && other.result == result;
  }
...
class _SFDropdownRoute<T> extends PopupRoute<_SFDropdownRouteResult<T>> {
  _SFDropdownRoute({
    this.items,
    this.padding,
    this.buttonRect,
  ...
  final List<_SFMenuItem<T>> items;
...
class _SFDropdownRoutePage<T> extends StatelessWidget {
  const _SFDropdownRoutePage({
    Key key,
    this.route,
    this.constraints,
    this.items,
      ...
      child: Builder(
        builder: (BuildContext context) {
          return CustomSingleChildLayout(
            delegate: _SFDropdownMenuRouteLayout<T>(
              buttonRect: buttonRect,
              route: route,
...
class _SFMenuItem<T> extends SingleChildRenderObjectWidget {
  const _SFMenuItem({
    Key key,
  ...
  final SFDropdownMenuItem<T> item;

  @override
  RenderObject createRenderObject(BuildContext context) {
    return _SFRenderMenuItem(onLayout);
  }

  @override
  void updateRenderObject(
      BuildContext context, covariant _SFRenderMenuItem renderObject) {
    renderObject.onLayout = onLayout;
  }
}

class _SFRenderMenuItem extends RenderProxyBox {
  _SFRenderMenuItem(this.onLayout, [RenderBox child])
      : assert(onLayout != null),
        super(child);
...
class _SFDropdownMenuItemContainer extends StatelessWidget {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const _SFDropdownMenuItemContainer({
    Key key,
    @required this.child,
  })  : assert(child != null),
        super(key: key);
...
/// An item in a menu created by a [SFDropdownButton].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class SFDropdownMenuItem<T> extends _SFDropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const SFDropdownMenuItem({
    Key key,
    this.onTap,
    this.value,
  ...
  ///
  /// Eventually returned in a call to [SFDropdownButton.onChanged].
  final T value;
}

/// An inherited widget that causes any descendant [SFDropdownButton]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [SFDropdownButton] widgets placed within material data tables, as
/// required by the material design specification.
class SFDropdownButtonHideUnderline extends InheritedWidget {
  /// Creates a [SFDropdownButtonHideUnderline]. A non-null [child] must
  /// be given.
  const SFDropdownButtonHideUnderline({
    Key key,
    @required Widget child,
  })  : assert(child != null),
        super(key: key, child: child);

  /// Returns whether the underline of [SFDropdownButton] widgets should
  /// be hidden.
  static bool at(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<
            SFDropdownButtonHideUnderline>() !=
        null;
  }

  @override
  bool updateShouldNotify(SFDropdownButtonHideUnderline oldWidget) => false;
}
...
/// Typically, an enum is used. Each [SFDropdownMenuItem] in [items] must be
...
/// See also:
///
///  * [SFDropdownMenuItem], the class used to represent the [items].
///  * [SFDropdownButtonHideUnderline], which prevents its descendant dropdown buttons
///    from displaying their underlines.
///  * [RaisedButton], [FlatButton], ordinary buttons that trigger a single action.
///  * <https://material.io/design/components/menus.html#dropdown-menu>
class SFDropdownButton<T> extends StatefulWidget {
  ...
  SFDropdownButton({
    ...
    // DropdownButtonFormField.
          ...
          items == null ||
              items.isEmpty ||
              value == null ||
              items.where((SFDropdownMenuItem<T> item) {
  ...
  /// be displayed.
  final List<SFDropdownMenuItem<T>> items;

  /// The value of the currently selected [SFDropdownMenuItem].
  ///
  ...
  /// [SFDropdownMenuItem]s in [items].
  ///
  /// When a [SFDropdownMenuItem] is selected, the widget that will be displayed
  /// from the list corresponds to the [SFDropdownMenuItem] of the same index
  /// in [items].
  ///
  /// {@tool dartpad --template=stateful_widget_scaffold}
  ///
  /// This sample shows a `DropdownButton` with a button with [Text] that
  /// corresponds to but is unique from [SFDropdownMenuItem].
  ///
  /// ```dart
  ...
  /// {@end-tool}
  ///
  /// If this callback is null, the [SFDropdownMenuItem] from [items]
  /// that matches [value] will be displayed.
  final SFDropdownButtonBuilder selectedItemBuilder;
  ...
  @override
  _SFDropdownButtonState<T> createState() => _SFDropdownButtonState<T>();
}

class _SFDropdownButtonState<T> extends State<SFDropdownButton<T>>
  ...
  @override
  void didUpdateWidget(SFDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
        widget.items
                .where(
                    (SFDropdownMenuItem<T> item) => item.value == widget.value)
                .length ==
            1);
    _selectedIndex = null;
  ...
  void _handleTap() {
    ...
    final menuItems = List<_SFMenuItem<T>>(widget.items.length);
    for (var index = 0; index < widget.items.length; index += 1) {
      menuItems[index] = _SFMenuItem<T>(
    ...
    _dropdownRoute = _SFDropdownRoute<T>(
      items: menuItems,
    ...
    Navigator.push(context, _dropdownRoute)
        .then<void>((_SFDropdownRouteResult<T> newValue) {
      _removeDropdownRoute();
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.hint != null || (!_enabled && widget.disabledHint != null)) {
      ...
      if (widget.selectedItemBuilder == null) {
        displayedHint = _SFDropdownMenuItemContainer(child: displayedHint);
      }
    ...
    if (!SFDropdownButtonHideUnderline.at(context)) {
...
/// A convenience widget that makes a [SFDropdownButton] into a [FormField].
class SFDropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [SFDropdownButton] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  SFDropdownButtonFormField({
    Key key,
    @required List<SFDropdownMenuItem<T>> items,
    SFDropdownButtonBuilder selectedItemBuilder,
    T value,
    Widget hint,
    Widget disabledHint,
    @required this.onChanged,
    ...
    bool autovalidate = false,
        ...
        super(
          ...
          builder: (FormFieldState<T> field) {
            final state = field as _SFDropdownButtonFormFieldState<T>;
            final decorationArg =
            ...
            return Focus(
              ...
              child: Builder(builder: (BuildContext context) {
                return InputDecorator(
                  ...
                  child: SFDropdownButtonHideUnderline(
                    child: SFDropdownButton<T>(
                      items: items,
  ...
  @override
  FormFieldState<T> createState() => _SFDropdownButtonFormFieldState<T>();
}

class _SFDropdownButtonFormFieldState<T> extends FormFieldState<T> {
  @override
  SFDropdownButtonFormField<T> get widget =>
      super.widget as SFDropdownButtonFormField<T>;
  ...
  @override
  void didUpdateWidget(SFDropdownButtonFormField<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
